...
char *url_easy_escape(const char *string, int inlength)
{
...
char *url_easy_unescape(const char *string, int length, int *olen)
{
  ...
  while(--alloc > 0) {
    ...
    if(('%' == in) && isxdigit(string[1]) && isxdigit(string[2])) {
      ...
      string+=2;
      alloc-=2;
    } else if ('+' == in) { //有些escape会把 space 编码成 +, 这里进行特殊处理,比如py的libcurl.quote_plus
	in = ' ';
    }
