...
INLINE void dealloc_funp(funptr_t *fp)
{
...
INLINE void free_funp(funptr_t *fp)
{
	fp->hdr.ref--;
	if (fp->hdr.ref > 0)
	{
...
INLINE void f_extract_range(int code)
{
	int from, len;
...
INLINE funptr_t *make_efun_funp(int opcode, svalue_t *args)
{
	funptr_t *fp;
...
INLINE funptr_t *make_lfun_funp(int index, svalue_t *args)
{
	funptr_t *fp;
...
INLINE funptr_t *make_simul_funp(int index, svalue_t *args)
{
	funptr_t *fp;
...
INLINE funptr_t *make_functional_funp(short num_arg, short num_local, short len, svalue_t *args, int flag)
{
	funptr_t *fp;
	...
	current_prog->func_ref++;
		...
		{
			printf("QTZ_ServerEngine:Add function ref /%s: now %i\n", current_prog->name, current_prog->func_ref);
		}
	#endif 
...
INLINE void push_refed_funp(funptr_t *fp)
{
	sp++;
	sp->type = T_FUNCTION;
	sp->u.fp = fp;
...
INLINE void push_funp(funptr_t *fp)
{
	sp++;
	sp->type = T_FUNCTION;
	sp->u.fp = fp;
...
INLINE void f_function_constructor()
{
	funptr_t *fp = 0;
	int kind;
	...
	switch (kind)
	{
		...
		default:
			...
			fatal("Can't not make unknown type of function pointer.\n");
	}
	push_refed_funp(fp);
}
