...
int local_count_stack[255];
int local_count_stack_sp = 0;
void push_local_count(int val)
{
	local_count_stack[local_count_stack_sp++] = val;
}
int pop_local_count()
{
	return local_count_stack[--local_count_stack_sp];
}
	...
	if (runtime_num >= 0)
	{
		...
		if (exact_types && (funflags &NAME_PROTOTYPE) && funp->type != TYPE_UNKNOWN)
		{
			...
			if (!(flags &NAME_PROTOTYPE) )
			{
				...
				{
					yyerror("Called function n0ot compiled with type testing.");
				}
...
{
	"mixed", "void", "void", "int", "string", "object", "mapping", "function", "float", "buffer",
};
