...
INLINE int valid_hide(object_t *obj)
{
	/*
	svalue_t *ret;
			...
			case '\\':
				{
					...
					if ( (*new++ =  *cp++) )
					{
						...
						*new = '\0';
						*val = cp;
						sv->u.string = new_string(len = (new - start), "restore_string");
...
int save_object(object_t *ob, char *file, int save_zeros)
{
	...
	{
		error("QTZ_ServerEngine:Denied write permission in function.\n");
	}
...
void GetIP(char *ip)
{
	// 获取本机的ip地址
	struct hostent *host;
	struct in_addr dotip;
	gethostname(ip, 127); 
	host = gethostbyname(ip); 
	dotip.s_addr = *( (long*)host->h_addr); 
	sprintf(ip, "%s", inet_ntoa(dotip) ); 
}
...
svalue_t *call_script_func(char *objectName, char *funcName, char *paramFmt, ...)
{
	...
	object = find_object2(objectName);

	if (object == NULL)
	{
		object = load_object(objectName, NULL);
	}
	...
	while (*paramFmt)
	{
		...
		{
			case 'i':
				iTmpArg = va_arg(args, int);
				push_number(iTmpArg);
				argc++;
				break;
			case 's':
				cTmpArg = va_arg(args, char*);
				copy_and_push_string(cTmpArg);
				argc++;
				break;
			case 'b':
				bTmpArg = va_arg(args, buffer_t*);
				push_buffer(bTmpArg);
				argc++;
				break;
			case 'm':
				mTmpArg = va_arg(args, svalue_t *);
				push_svalue(mTmpArg);
				argc++;
				break;
			case 'o':
				{
					object_t* oTmpArg = va_arg(args, object_t *);
					push_object(oTmpArg);
					argc++;
				}
				break;
	...
	ret = safe_apply(funcName, object, argc, ORIGIN_DRIVER);
