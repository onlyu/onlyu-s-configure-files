...
INLINE void push_undefineds(int num)
{
...
INLINE void copy_and_push_string(char *p)
{
	STACK_CHECK;
	sp->type = T_STRING;
...
INLINE void share_and_push_string(char *p)
{
	STACK_CHECK;
	sp->type = T_STRING;
...
svalue_t *call_function_pointer(funptr_t *funp, int num_arg)
{
	...
	{
		error("QTZ_SE:Owner (/%s) of function pointer is destructed.\n", funp->hdr.owner->name);
	}
	...
	switch (funp->hdr.type)
	{
		...
		default:
			error("QTZ_SE:Unsupported function pointer type.\n");
	}
	free_svalue(&apply_ret_value, "call_function_pointer");
	apply_ret_value =  *sp--;
	remove_fake_frame();
		...
		switch (instruction)
		{
			case F_PUSH:
				...
				while (n--)
				{
					...
					case PUSH_LOCAL:
						lval = fp + (i &PUSH_MASK);
						DEBUG_CHECK( (fp - lval) >= csp->num_local_variables, "Tried to push non-existent local\n");
			...
			case F_CALL_FUNCTION_BY_ADDRESS:
				{
					 ...
					 * current_object.
					 */
					DEBUG_CHECK(offset >= current_object->prog->num_functions_total, "Illegal function index\n");
...
svalue_t *safe_apply(char *fun, object_t *ob, int num_arg, int where)
{
	svalue_t *ret;
	error_context_t econ;
	...
	for (p = &control_stack[0]; p < csp; p++)
	{
		switch (p[0].framekind &FRAME_MASK)
		{
			case FRAME_FUNCTION:
				...
				break;
			case FRAME_FUNP:
				debug_message("'<function>' in '%30s' at %s\n",
				...
				break;
			case FRAME_FAKE:
				debug_message("'<function>' in '%30s' at %20s') %s\n",
						p[1].ob->name, get_line_number(p[1].pc, p[1].prog) );
				num_arg =  - 1;
				break;
			case FRAME_CATCH:
 ...
 */
svalue_t *apply_master_ob(char *fun, int num_arg)
{
	IF_DEBUG(svalue_t *expected_sp);
