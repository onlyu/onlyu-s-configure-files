...
char *tree_srch(ppr_tree, pfi_compare, pc_user)tree *ppr_tree;
...
void tree_add(ppr_tree, pfi_compare, pc_user, pfi_delete)tree **ppr_tree;
int(*pfi_compare) ();
char *pc_user;
int(*pfi_delete) ();
{
...
static void sprout(ppr, pc_data, pi_balance, pfi_compare, pfi_delete)tree **ppr;
char *pc_data;
int *pi_balance;
int(*pfi_compare) (void *, void*);
int(*pfi_delete) (void*);
...
int tree_delete(ppr_p, pfi_compare, pc_user, pfi_uar)tree **ppr_p;
int(*pfi_compare) ();
char *pc_user;
int(*pfi_uar) ();
{
	...
	return delete (ppr_p, pfi_compare, pc_user, pfi_uar,  &i_balance, &i_uar_called);
}
...
static int delete (ppr_p, pfi_compare, pc_user, pfi_uar, pi_balance, pi_uar_called)tree **ppr_p;
int(*pfi_compare) (void *, void*);
char *pc_user;
int(*pfi_uar) (void*);
int *pi_balance;
int *pi_uar_called;
{
	...
	if (i_comp > 0)
	{
		i_ret = delete (&(*ppr_p)->tree_l, pfi_compare, pc_user, pfi_uar, pi_balance, pi_uar_called);
		if (*pi_balance)
		{
	...
	{
		i_ret = delete (&(*ppr_p)->tree_r, pfi_compare, pc_user, pfi_uar, pi_balance, pi_uar_called);
		if (*pi_balance)
		{
...
static void del(ppr_r, pi_balance, ppr_q, pfi_uar, pi_uar_called)tree **ppr_r;
int *pi_balance;
tree **ppr_q;
int(*pfi_uar) (void*);
int *pi_uar_called;
...
int tree_trav(ppr_tree, pfi_uar)tree **ppr_tree;
int(*pfi_uar) (void*);
{
	if (! *ppr_tree)
	{
...
void tree_mung(ppr_tree, pfi_uar)tree **ppr_tree;
int(*pfi_uar) (void*);
{
	if (*ppr_tree)
	{
