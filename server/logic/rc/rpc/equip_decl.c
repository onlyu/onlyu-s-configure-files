...
void rpc_client_equip_item_info(int uid, int item, int qianghuaCnt, class equip_value_t *values, string *baoshis);

// 装备相关的信息
void rpc_client_equip_desc(int uid, string type, int grade, int qianghuaVal, int locate, int color, class equip_value_t *baseValues);

// 强化装备
void rpc_server_equip_qianghua(object user, int item);

// 登录时发
void rpc_client_equip_qianghua_cd(int uid, int sec, int maxSec);

// 强化
void rpc_client_equip_qianghua_info(int uid, int item, int cash);
void rpc_server_equip_qianghua_info_req(object user, int item);

// 洗练
// normalCash : 普通洗练金钱
// directCash : 定向洗练金钱
void rpc_client_equip_xilian_info(int uid, int item, int normalCash, int directCash, class equip_value_t *attr);
...
// 免费个数
void rpc_client_baoshi_free_fetch_cnt(int uid, int cnt);
